...
exports.book_delete_get = function(req, res, next) {
   async.parallel({
        ...
        if (err) { return next(err); }
        //Successful, so render
        res.render('book_delete', { title: 'Delete book', book: results.book, book_inst: results.book_inst} );
    });
...
exports.book_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err); }
        //Success
        if (results.book_inst.length > 0) {
            //Author has books. Render in same way as for GET route.
            res.render('book_delete', { ttitle: 'Delete book', book: results.book, book_inst: results.book_inst} );
            return;
        }
        else {
